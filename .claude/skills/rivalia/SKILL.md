...
- **MAILBOX / Postman Quest** (Andrea la sta completando, set 2026). Reward: parcel 10gp / letter 5gp, accesso alle **Royal Mailbox** (vicino ai posti di caccia) e mailbox in zone pericolose. Uso: a una mailbox spedisci un parcel a te stesso → loot al depot **senza tornare in città**. **Posizioni mailbox su Rivalia NON documentate** (assenti nei dati locali + wiki JS; verificare in-game via icona minimap o Discord). Certo: **ogni città ha mailbox** (dentro/vicino al depot). **Candidate 7.4-era** (da TibiaWiki Mailbox, coord mapper → gioco convertite; ⚠️ VANILLA, placement/floor Rivalia da verificare, e le post-7.4 tipo Fury Dungeon/Nargor/Shadowthorn/Talahu/Vengoth/Zao/Formorgar/Beregar/Rascacoon quasi certamente ASSENTI):
  - **Non-locked (usabili senza Postman completa):** Fibula città; **Folda cima montagna ~(32009,31564,z7)** (= supply Frost Troll spear); Outlaw Camp sotto ~(32653,32202,z8).
  - **Royal (serve Postman completa):** **Minotaur Pyramid Darashia ~(33307,32292,z7)** [🥇 banking farm Scarab/Mummy/Mino]; Cyclopolis ~(33271,31656,z8); Drefia ~(33019,32447,z7); Orc Fortress ~(33018,31778,z7); Mintwallin nord ~(32423,32095,z15); Kazordoon Dwarf Mines + 1 superficie ~(32535,31964,z7) [mining].
  - **Port Hope / Nivarya**: NON documentate (custom), solo esplorazione in-game.
  - **TODO possibile: layer "mailbox" sulla reverse map** (skill `rivalia-map`) quando Andrea raccoglie/conferma le coord in-game.
...
- **Royal Paladin — Livello 46, MID-GAME** (aggiornato 11 set 2026). **Distance Fighting 74, Shielding 60, Magic Level 13.** (era lv40/D71/S57 il 2 set; lv30/D64/S53 il 27 lug.) Distance = skill primaria. ⚠️ Il default `CHAR_DEF` nella reverse map è fermo a D64/S53 (lug) → rigenerare/ripubblicare (skill `rivalia-map`).
- **Ora caccia ANCHE con CROSSBOW** (non più solo spear/kiting puro): setup ibrido mid-game. Con mlvl 13 può usare **Conjure Bolt** (`exevo con mort`, 70 mana, mlvl6) per l'ammo; **a mlvl 14 sblocca Conjure Power Bolt** (`exevo con vis`, 200 mana, Premium) — power bolt = ammo molto più forte, è a **1 solo mlvl di distanza**.
- **PROMOTION → Royal Paladin (custom Rivalia): +25% damage — CONFERMATO da Andrea in-game (11 set 2026).** Meccanica CUSTOM (vanilla la promotion NON dà danno). Le tabelle danno "SPEAR" qui sotto vanno **moltiplicate ×1.25** (char promosso). Vale anche per crossbow/bolt.
...
- **Crossbow ATTIVO** (aggiornato 11 set): non più deprioritizzato, lo usa in caccia. Una crossbow da loot è candidata per Item Attributes (Double Shot / Attack / Critical la rendono un vero upgrade). Ammo via Conjure Bolt (mlvl6) ora, Power Bolt a mlvl14.
