...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* DESKTOP-neutral: the creature-sheet wrapper is transparent (children float exactly as
     before) and the mobile-only chrome (combined header + 🐉 FAB) is hidden. */
  #crSheet{display:contents}
  #crSheetHd,#crFab,#crBar{display:none}

  /* ===== MOBILE — full-screen map + slide-up bottom sheet (like Maps apps) ===== */
  @media (max-width:820px){
    html,body{height:100%;overflow:hidden}
    #app{display:block;height:100dvh;overflow:hidden}
    /* map fills the whole screen */
    #mapwrap{position:fixed;inset:0;width:100vw;height:100dvh}
    /* the hover scanner + ghost/level overlays are mouse-only → hide on touch layout */
    #scan,#ghostbox{display:none!important}
    #floors{top:calc(env(safe-area-inset-top) + 10px);left:10px;padding:5px;max-height:58vh}
    #readout{font-size:11px;padding:6px 10px;bottom:78px;left:10px;z-index:600}

    /* ---- SIDEBAR = bottom sheet: peeks at the bottom, tap the handle to expand ---- */
    #side{
      position:fixed;left:0;right:0;bottom:0;width:auto;z-index:1200;
      background:var(--panel);border:none;border-top:1px solid var(--line);
      border-radius:18px 18px 0 0;box-shadow:0 -10px 34px rgba(0,0,0,.55);
      max-height:86dvh;overflow-y:auto;-webkit-overflow-scrolling:touch;
      padding-bottom:calc(env(safe-area-inset-bottom) + 14px);
      transform:translateY(calc(100% - 62px));           /* PEEK: only the handle shows */
    ...
    #side h1{
      position:sticky;top:0;z-index:6;background:var(--panel);cursor:pointer;
      border-radius:18px 18px 0 0;padding:20px 16px 12px;margin:0;
      font-size:15px;display:flex;justify-content:space-between;align-items:center;
    }
    #side h1::before{                                     /* grabber pill */
      content:"";position:absolute;top:7px;left:50%;transform:translateX(-50%);
      width:40px;height:4px;border-radius:2px;background:var(--line);
    }
    #side h1::after{content:"▴";font-size:15px;color:var(--dim)}
    #side.open h1::after{content:"▾"}
    #side h1 small{display:none}                          /* hide the long subtitle on the handle */
    /* roomier controls when the sheet is open */
    .sec{padding:14px 16px}
    .btnrow{gap:8px;margin-top:10px}
    .btn,.itab,.pktab,.ptab,.reltab{min-height:42px;font-size:13px}
    #search,#pkQ,input[type=text],input[type=search]{min-height:42px;font-size:15px}  /* 15px+ stops iOS zoom-on-focus */
    .cfg-note,.ins-note{font-size:11px}
    /* creature results list: on desktop it's flex:1 in a full-height column; inside the
       bottom sheet that collapses to ~0 → give it a real, scrollable height so results show */
    #clist{flex:none;max-height:44vh;min-height:120px;overflow-y:auto;
      border-top:1px solid var(--line);margin-top:4px}
    #clist:empty{display:none}
    .crow{padding:10px 8px}          /* bigger tap rows */

    /* ---- 🐉 creature FAB: mobile entry point to search creatures. Anchored bottom-right,
       clear of the Leaflet zoom (hidden on mobile) and the floor column; rises above the
       result bar when it's shown so it's always tappable regardless of map zoom/pan. ---- */
    #crFab{display:flex;align-items:center;justify-content:center;position:fixed;
      right:14px;bottom:calc(env(safe-area-inset-bottom) + 16px);z-index:1400;
      width:56px;height:56px;border-radius:50%;font-size:26px;
      background:var(--acc);color:#111;border:none;box-shadow:0 4px 16px rgba(0,0,0,.5);cursor:pointer}
    #crFab.lifted{bottom:calc(env(safe-area-inset-bottom) + 84px)}   /* rise above the result bar */

    /* ---- slim RESULT BAR: shown after picking a creature; map stays visible above it ---- */
    #crBar{display:flex;align-items:center;gap:8px;position:fixed;left:8px;right:8px;
      bottom:calc(env(safe-area-inset-bottom) + 10px);z-index:1250;
      background:var(--panel);border:1px solid var(--line);border-radius:14px;
      padding:10px 10px 10px 14px;box-shadow:0 6px 24px rgba(0,0,0,.55)}
    #crBar.show{display:flex}
    #crBarInfo{flex:1;min-width:0;font-size:13px;line-height:1.35;overflow:hidden}
    #crBarInfo b{font-size:14px}
    #crBarInfo .vbadge{padding:1px 8px;font-size:11px;margin-left:2px}
    #crBarMore{flex:none;background:var(--acc);color:#111;border:none;border-radius:9px;
      padding:8px 11px;font-size:12px;font-weight:700;cursor:pointer;min-height:38px}
    #crBarX{flex:none;background:var(--panel2);border:1px solid var(--line);color:var(--txt);
      width:38px;height:38px;border-radius:9px;font-size:15px;cursor:pointer}

    /* ---- combined creature sheet: expanded details (verdict + stats). Leaves the top of the
       map visible so you keep spatial context; opens only when "Dettagli" is tapped. ---- */
    #crSheet{display:none;position:fixed;left:0;right:0;bottom:0;z-index:1300;
      background:var(--panel);border-top:1px solid var(--line);border-radius:18px 18px 0 0;
      max-height:70dvh;overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;
      box-shadow:0 -10px 34px rgba(0,0,0,.6);padding-bottom:calc(env(safe-area-inset-bottom) + 16px);
      animation:crUp .28s cubic-bezier(.32,.72,0,1)}
    #crSheet.show{display:block}
    @keyframes crUp{from{transform:translateY(100%)}to{transform:translateY(0)}}
    #crSheetHd{display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:3;
      background:var(--panel);padding:16px 16px 10px;font-size:15px;font-weight:800;border-radius:18px 18px 0 0}
    #crSheetHd::before{content:"";position:absolute;top:7px;left:50%;transform:translateX(-50%);
      width:40px;height:4px;border-radius:2px;background:var(--line)}
    #crSheetHd button{background:var(--panel2);border:1px solid var(--line);color:var(--txt);
      width:34px;height:34px;border-radius:8px;font-size:16px;cursor:pointer}
    /* inside the combined sheet the two panels become plain stacked sections (no floating) */
    #crSheet #verdict,#crSheet #stats{position:static!important;width:auto!important;max-width:none!important;
      max-height:none!important;display:block!important;border:none;border-radius:0;box-shadow:none;
      background:none;overflow:visible;inset:auto}
    #crSheet #verdict{border-bottom:1px solid var(--line);padding-bottom:6px}
    #crSheet #verdict .hd,#crSheet #stats .shd{background:none;border-bottom:none;position:static}
    #crSheet #vClose,#crSheet #st-close{display:none}   /* one close button (in crSheetHd) is enough */
    .vmetrics{grid-template-columns:1fr 1fr}
    .stat-grid{grid-template-columns:1fr 1fr}

    /* ---- modals: full-bleed, sticky header/tabs, comfortable tap targets ---- */
    .ins-modal{align-items:flex-end}                      /* modal rises from the bottom */
    .ins-card{width:100vw!important;max-width:100vw;max-height:92dvh;border-radius:18px 18px 0 0}
    .ins-hd{position:sticky;top:0;z-index:6;padding:15px 16px}
    .ins-hd button{width:34px;height:34px;font-size:16px}
    .ins-tabs{flex-wrap:wrap;position:sticky;top:49px;z-index:5;background:var(--panel);padding:10px 12px 8px}
    .itab,.pktab,.reltab,.tktab{min-height:42px;font-size:12px;flex:1 1 30%}
    .ptab{min-height:42px;font-size:13px;flex:1 1 45%}
    ...
    #pkBody .exp{font-size:15px}                          /* bigger expand tap target */
  }
</style>
</head>
<body>
